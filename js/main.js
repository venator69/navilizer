...
const algoDesc = {
  ...
  qlearning: `<strong>Tabular Q-learning</strong> learns action values <em>Q(s,a)</em> by trial and error. On earlier stage of the training, the action is determined by epsilon-greedy policy. Which means that the agent will explore the environment randomly with a probability of epsilon = 0.35. The epsilon is the decreased using a decay factor 0.97 per episode. When goals is reached, the heatmap of the Q-values is updated. .<strong>Local best</strong> shows the shortest successful episode so far, measured in environment <em>steps</em> (<code>out.steps</code> when the goal is reached)—it only improves when a new episode hits the goal in fewer steps than any previous one. The maximum number of steps per episode is 400.`,
  qnet: `<strong>Deep Q-network</strong>: Phase 1 fills a replay buffer (no weight updates). The agent will explore the environment randomly with a probability of epsilon = 0.3. The epsilon is the decreased using a decay factor 0.95 per episode. The agent will collect the transitions and store them in the buffer. The buffer is used to train the Q-network in the second phase. Gradient steps is capped on 2000 steps.<strong>Local best</strong> is the minimum rollout step count among acquisition episodes that reach the goal. With acquisition viz on, rollouts animate and the log prints buffer tails; with it off, phase 1 stays off-canvas. Phase 2 runs TD replay; heatmaps and the greedy path appear after training. The maximum number of steps per episode is 400. so the buffer size is based on the number of episodes and the maximum number of steps per episode(episodes * 400).`,
};
